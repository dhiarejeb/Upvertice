  ...
  ngOnInit(): void {
    this.route.params.subscribe((params) => {
      ...
      if (id) {
        ...
        this.error = true;
        this.loading = false;
        this.toastService.error('Invalid providership ID.', 'Error');
      }
    });
  }
    ...
    this.providershipService
      ...
      .pipe(
        catchError((err) => {
          ...
          this.error = true;
          this.loading = false;
          this.toastService.error('Failed to load providership details. Please try again.', 'Error');
          return of(null);
        })
      )
      .subscribe((data) => {
        ...
        this.loading = false;
        if (data) {
          this.toastService.success('Providership details loaded successfully!', 'Success');
          setTimeout(() => {
            this.renderCharts();
          }, 100);
        }
